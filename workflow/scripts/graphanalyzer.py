...
def fillWithMetas(csv, metas):
    ...
    csv_edit["Accession"] = csv_edit[
        "Genome"
    ]  # copy to allow future edits of "Genome".
    csv_edit[
        "FullClass"
    ] = None  # to save the "full classification" as stored in metas.Classification.
    ...
    csv_edit[
        "BaltimoreGroup"
    ] = None  # Not a real taxonomy level - to be filled with info from metas, matching the accession number.
    csv_edit[
        "Realm"
    ] = None  # Or "domain" - to be filled with info from metas, matching the accession number.
    ...
    csv_edit[
        "Subfamily"
    ] = None  # to be filled with info from metas, matching the accession number.
    ...
    for index, row in csv_edit.iterrows():
        ...
        if len(matches) == 0:
            ...
            consoleout(
                "error", "More than 1 match in filler() function!"
            )  # stop the program.
        else:
            ...
            csv_edit.at[
                index, "Species"
            ] = match.Description  # match.Description contains only the species name.
            ...
            def scroll_linealogy(
                lin, suff
            ):  # function to find term with correspondent suffix in linealogy (list).
                cnt = 0
                ...
                for x in lin:
                    if x.endswith(suff):
                        ...
                        if cnt > 1:  # There SHOULD be only one match for every suffix:
                            consoleout(
                                "warning",
                                f'More than 1 term in linealogy with suffix "{suff}"! Taking the last.',
                            )
                        got = x
                return got

            csv_edit.at[index, "Subrealm"] = scroll_linealogy(
                linealogy, lvlsuffix["Subrealm"]
            )
            csv_edit.at[index, "Kingdom"] = scroll_linealogy(
                linealogy, lvlsuffix["Kingdom"]
            )
            csv_edit.at[index, "Subkingdom"] = scroll_linealogy(
                linealogy, lvlsuffix["Subkingdom"]
            )
            csv_edit.at[index, "Phylum"] = scroll_linealogy(
                linealogy, lvlsuffix["Phylum"]
            )
            csv_edit.at[index, "Subphylum"] = scroll_linealogy(
                linealogy, lvlsuffix["Subphylum"]
            )
            csv_edit.at[index, "Class"] = scroll_linealogy(
                linealogy, lvlsuffix["Class"]
            )
            csv_edit.at[index, "Subclass"] = scroll_linealogy(
                linealogy, lvlsuffix["Subclass"]
            )
            csv_edit.at[index, "Order"] = scroll_linealogy(
                linealogy, lvlsuffix["Order"]
            )
            csv_edit.at[index, "Suborder"] = scroll_linealogy(
                linealogy, lvlsuffix["Suborder"]
            )
...
def clusterExtractor(graph, csv_edit, output_path, string_suffix, prefix):
    ...
    def insertReference(csv_edit, results, scaffold, closer, weight, level, status, vc):
        ...
        if level != "F" and level != "G" and level != "A" and level != "AF":
            baltim = csv_edit.loc[
                csv_edit["Genome"] == closer, "BaltimoreGroup"
            ].values[0]
            ...
            subfamily = csv_edit.loc[csv_edit["Genome"] == closer, "Subfamily"].values[
                0
            ]
    ...
    logging.basicConfig(
        filename=output_path + "graphanalyzer_" + string_suffix + ".log",
        filemode="w",
        level=logging.INFO,  # level sets the threshold
    ...
    for index, row in scaffolds_ingraph.iterrows():
        ...
        if (
            status == "Clustered"
            or status == "Outlier"
            or status == "Clustered/Singleton"
            or "Overlap" in status
        ):
            vc = ""  # scaffold's viral cluster or subcluster
            sameclustered_list = (
                []
            )  # other genomes/scaffolds in the same cluster/subcluster
            ...
            if status == "Outlier":  # not clustered but connected
                ...
                for (
                    vc
                ) in vc_list:  # extract the rows and glue them together with the others
                    sameclustered = pnd.concat(
                        [
                            sameclustered,
                            csv_edit[csv_edit["VC"].str.startswith(vc + "_")],
                        ]
                    )
                ...
                consoleout(
                    "error", "Strange level found during the extraction of the cluster."
                )
            ...
            logging.info(
                "Are all sc connected to scaffold?: %s"
                % set(sameclustered_list).issubset(neighbors_list)
            )
            ...
            connected_list = sorted(
                connected_list, key=operator.itemgetter("weight"), reverse=True
            )  # reverse for descending order

            # EXTRACT 'CLOSER_NODE' (= the haviest connected-sameclustered)
            # note that closer_node could be another scaffold!
            try:
                closer_node = connected_list[0][
                    "scaffold"
                ]  # to store the node connected with the heavier edge
            ...
            # 'CLOSER_REF' SEARCH
            closer_ref = (
                "n.a."  # store the reference genome connected with the heavier edge
            )
            closer_ref_w = (
                0.0  # store the heavier egde that connect to a reference genome
            )
            ...
            if closer_ref != "n.a.":
                ...
                if closer_ref != "n.a.":
                    ...
                    logging.warning(
                        "NO reference in connected sameclustered AND NO reference in neighbors."
                    )
                    ...
                    genomes_connected = list(
                        net.node_connected_component(graph, row.Genome)
                    )
            ...
            logging.consoleout(
                "error",
                "Found a strange VCStatus! Maybe it's a Singleton? Singletons should not be present inside the graph.",
            )
    ...
    for index, row in results.iterrows():
        ...
        elif row.Level.startswith("C") and (
            row.Status == "Clustered/Singleton" or "Overlap" in row.Status
        ):
    ...
    file_report = open(
        output_path + "custom_taxonomy_table_" + parameters.suffix + "_mqc.txt", "w"
    )
...
def get_curr_color(
    ...
    # Overlap
        ...
        valid_scaff_VCs = (
            scaff_Status.replace("Overlap (", "").replace(")", "").split("/")
        )
    ...
    if curr_Status == "Clustered" or curr_Status == "Clustered/Singleton":  # VC_z_k
        # Overlap
        if (
            "Overlap" in scaff_Status
        ):  # 'valid_scaff_VCs' eg ['VC_589_', 'VC_590_', 'VC_591_']
    ...
    # Overlap
        ...
        valid_curr_VCs = (
            curr_Status.replace("Overlap (", "").replace(")", "").split("/")
        )
    ...
    if scaff_Status == "Clustered" and (
        curr_Status == "Clustered" or curr_Status == "Clustered/Singleton"
    ):
        if (("VC_" + scaff_VC.split("_")[1] + "_") in curr_VC) and (
            curr_VC != scaff_VC
        ):
            curr_color = col_outer
...
def subgraph_generation2(arguments):
    ...
    for scaffold in scaffolds:
        ...
        if scaff_Status == "Singleton" or scaff_Level == "G":
            consoleout(
                "error",
                "There shouldn't be 'G' scaffolds at this point. Contact the developer.",
            )
        ...
        if view == "3d" or view == "3D":
            sview_pos = net.drawing.layout.spring_layout(
                sview_graph, weight="weight", dim=3, seed=1
            )
        else:
            sview_pos = net.drawing.layout.spring_layout(
                sview_graph, weight="weight", dim=2, seed=1
            )
        ...
        layout = go.Layout(
            ...
            annotations=[
                dict(
                    text='User guide available at <a href="https://www.github.com/lazzarigioele/graphanalyzer/">github.com/lazzarigioele/graphanalyzer</a>.'
        ...
        for node in sview_graph:  # here node is a string !
            attrs = {}  # a dict.
            if node == scaffold:
                attrs = {node: {"A6_Weight": "origin"}}
            else:
                attrs = {
                    node: {
                        "A6_Weight": str(
                            round(sview_graph[scaffold][node]["weight"], 1)
                        )
                    }
                }
        ...
        for node in sview_graph:  # here node is a string !
            ...
            if curr_Type == "Scaffold":
                ...
                curr_shape = (
                    "diamond" if view == "3d" or view == "3D" else "star-triangle-up"
                )
            else:
                consoleout(
                    "error", "Strange A0_Type when determining the node's shape."
                )
            nodes_shape.append(curr_shape)

            # get the color of this node:
            curr_color = get_curr_color(
                scaff_Status,
                scaff_VC,
                scaff_Accession,
                curr_Status,
        ...
        if view == "3d" or view == "3D":
            ...
            node_trace = go.Scatter(
                ...
                hoverinfo="text",
                marker=dict(
                    size=12, color=nodes_color, line_width=1, symbol=nodes_shape
                ),
            )
        ...
        print(
            f"W#{worker}-PID {os.getpid()}: {scaffold} #E {nedges} "
            + f"[Ch {int(alloc_child/10**6)} MB; In {int((alloc_free-alloc_child)/10**6)} MB]         ",
            end="\r",
        )
...
def subgraphCreator(
    ...
    if os.path.isdir(desired_path) == False:  # if it's not been created yet:
        try:
            ...
            consoleout(
                "error", "Can't create the output sub-folder '%s'. " % desired_path
            )
    ...
    if len(scaffolds_ingraph) % nthreads != 0:
        nscaffodls_inchunk += 1
    chunks = [
        scaffolds_ingraph[x * nscaffodls_inchunk : (x + 1) * nscaffodls_inchunk]
...
if __name__ == "__main__":
    ...
    parser_conventional.add_argument(
        "-v",
        "--version",
        action="version",
    ...
    if parameters.pickle != None:  # launch fast or debugging mode.
        ...
        with open(parameters.pickle, "rb") as filehandler:
            ...
            consoleout(
                "warning",
                "A pickle was specified. Now starting graphanalyzer from the plot generation step.",
            )
            ...
            consoleout(
                "okay",
                f"Starting to generate {len(scaffolds_ingraph)} subraphs with {parameters.threads} threads.",
            )
            ...
            consoleout(
                "okay",
                f"Generated {len(scaffolds_ingraph)} interactive subgraphs in {time.time() - start_time} s.",
            )
        ...
        consoleout(
            "error",
            f"Impossible to launch graphanalyzer.py with the pickle provided. Will halt here.",
        )
    ...
    if os.path.isdir(parameters.output) == False:  # if it's not a folder:
        if os.path.isfile(parameters.output) == True:  # it's file:
            consoleout(
                "error",
                "Seems that --output passed as %s is a file and not a folder."
                % parameters.output,
            )
        else:  # it's a folder, but not yet created.
            consoleout(
                "error",
                "Seems that --output passed as %s doesn't exist." % parameters.output,
            )
    ...
    start_time = time.time()
    graph = net.read_edgelist(
        graph_table, nodetype=str, data=(("weight", float),), create_using=net.Graph()
    )
    consoleout(
        "okay",
        f"{parameters.graph} converted into a Graph object in {time.time() - start_time} s.",
    )
    alloc, _ = tracemalloc.get_traced_memory()
    ...
    consoleout(
        "okay", f"{parameters.csv} loaded into memory in {time.time() - start_time} s."
    )
    ...
    consoleout(
        "okay",
        f"{parameters.metas} loaded into memory in {time.time() - start_time} s.",
    )
    ...
    consoleout(
        "okay",
        f"List of {len(votus_ingraph)}/{len(votus)} vOTU contained in the graph got in {time.time() - start_time} s.",
    )
    ...
    consoleout(
        "okay",
        f"{parameters.csv} updated with INPHARED db in {time.time() - start_time} s.",
    )
    csv_edit.to_excel(
        parameters.output + "csv_edit_" + parameters.suffix + ".xlsx"
    )  # save the table!

    alloc, _ = tracemalloc.get_traced_memory()
    consoleout(
        "meminfo",
        f"{alloc / 10**6} MB after 'csv', 'metas', 'votus/votus_ingraph' and 'csv_edit' loading.",
    )

    # 2nd PART:
    # Run the main algorithm. For every viral scaffold, get the most probable taxonomy:
    start_time = time.time()
    df_results = clusterExtractor(
        graph, csv_edit, parameters.output, parameters.suffix, parameters.prefix
    )
    ...
    consoleout(
        "okay",
        f"Taxonomy of {len(votus)} vOTUs (C:{nC}; N:{nN}; G:{nG}; F:{nF}; A:{nA}) obtained in {time.time() - start_time} s.",
    )
    ...
    if curr_max_weight > max_weight:
        consoleout(
            "warning",
            f"Max weight here is > {max_weight} ({curr_max_weight}). {max_weight} will be used anyway. Please contact the developer.",
        )

    # first use 'csv_edit' and 'results' to fill 'graph' with useful attributes.
    graph, scaffolds_ingraph = addGraphAttributes(
        graph, csv_edit, df_results, parameters.prefix
    )
    consoleout("okay", f"Node attributes were added to the Graph objects.")
    alloc, _ = tracemalloc.get_traced_memory()
    consoleout(
        "meminfo",
        f"{alloc / 10**6} MB after 'arrows', 'scaffolds_ingraph', and 'graph' attributes loading.",
    )
    ...
    with open(parameters.output + parameters.suffix + ".pickle", "wb") as filehandler:
        ...
        consoleout(
            "okay",
            f"Saved {parameters.output + parameters.suffix + '.pickle'} for debugging purposes.",
        )
    ...
    consoleout(
        "okay",
        f"Starting to generate {len(votus_ingraph)} subraphs with {parameters.threads} threads.",
    )
    start_time = time.time()
    subgraphCreator(
        graph,
    ...
    consoleout(
        "okay",
        f"Generated {len(votus_ingraph)} interactive subgraphs in {time.time() - start_time} s.",
    )
