...
def create_sample_stats_virsorter2():
    ...
    """
    column_names = ['Sample','Raw_reads','QC_reads','Contigs','QC_contigs'
    ,'CheckV_Complete','CheckV_High-quality', 'CheckV_Medium-quality', 'CheckV_Low-quality', 'CheckV_Not-determined'
    ,'VS_dsDNAphage','VS_ssDNA','VS_NCLDV','VS_RNA','VS_lavidaviridae']

    standard = ["NaN"] * len(column_names)

    df=pd.DataFrame(columns=column_names)

    metaQuast_reads= pd.read_table('results/metaQUAST/combined_reference/transposed_report.tsv', usecols =["Assembly", "# contigs (>= 0 bp)"])
    metaQuast_reads['Assembly']= metaQuast_reads['Assembly'].str.replace('_contigs', '')
    metaQuast_reads= dict(zip(metaQuast_reads['Assembly'], metaQuast_reads['# contigs (>= 0 bp)']))
    ...
    for x in samples["sample"]:
        ...
        row[4] = getMetaSpades('results/checkv/virsorter2/'+x+'/filtered/filtered_combined.fna')
        #CheckV Score
...
def create_sample_stats_vibrant():
    ...
    column_names = ['Sample','Raw_reads','QC_reads','Contigs','QC_contigs'
    ,'CheckV_Complete','CheckV_High-quality', 'CheckV_Medium-quality', 'CheckV_Low-quality', 'CheckV_Not-determined'
    ,'fragment','low quality draft','medium quality draft','high quality draft','complete circular', 'lytic', 'lysogenic']
    ...
    for x in samples["sample"]:
        ...
        vibrant = get_vibrant_quality("results/vibrant/"+x+"/VIBRANT_contigs/VIBRANT_results_contigs/VIBRANT_genome_quality_contigs.tsv")
        raw_reads, filtered_reads = get_trimmed_report(x)
        row=standard
        #Sampe name
        row[0]= x
        ...
        row[2] = filtered_reads
        row[3] = metaQuast_reads[x]
        row[4] = getMetaSpades('results/checkv/vibrant/'+x+'/filtered/filtered_combined.fna')
        #CheckV Score
        row[5]=checkV['Complete']
        row[6]=checkV['High-quality']
        row[7]=checkV['Medium-quality']
...
def getCheckV(input):
    ...
    index= ['Not-determined', 'Low-quality', 'Medium-quality', 'High-quality', 'Complete', 'Total-checked']
...
def get_graphAnalyzer():
    ...
    """
    gA_df = pd.read_table("./results/graphanalyzer/results_vcontact2_vOTU_results.csv",sep=',', usecols =["Scaffold", "Accession", "Status", 'Family', 'Subfamily', 'Genus'])
    gA_df['indexNumber'] = [int(i[5:]) for i in gA_df['Scaffold']]
    gA_df.sort_values(by=['indexNumber'], ascending = [True], inplace = True, ignore_index=True)
...
def get_DRAMv(input):
    ...
    """
    DRAMv=pd.read_table(input ,sep='\t', usecols = ['contig_id', 'provirus', 'checkv_quality'])
def stats_vOTUs_virsorter2():
    """
    Function that creates the stats table of the vOTUs based on virsorter2 rerun.
    """
    column_names = ['vOTU_ID', 'provirus', 'Checkv_quality', 'Accession', "Status", 'Host', 'Subfamily', 'Genus']
    standard = ["NaN"] * len(column_names)
    vOTU_stats = pd.DataFrame(columns=column_names)
    gA_df = get_graphAnalyzer()
    cV_df = pd.read_table('results/checkv/vOTU/virsorter2/quality_summary.tsv', sep="\t", usecols=['contig_id', 'provirus', 'checkv_quality'])
...
def stats_vOTUs_vibrant():
    ...
    vOTU_stats = pd.DataFrame(columns=column_names)
    gA_df = get_graphAnalyzer()
    cV_df = pd.read_table('results/checkv/vOTU/vibrant/quality_summary.tsv', sep="\t", usecols=['contig_id', 'provirus', 'checkv_quality'])
    cV_df = cV_df.rename(columns={'contig_id': 'Scaffold'})
    result = pd.merge(gA_df, cV_df, on='Scaffold', how='outer')
...
def vOTU_AMG_stats():
    ...
    df = pd.DataFrame(columns=column_names)
    vibrant_amgs = pd.read_table('results/vibrant/vOTU/VIBRANT_vOTU_derep95_combined/VIBRANT_results_vOTU_derep95_combined/VIBRANT_AMG_individuals_vOTU_derep95_combined.tsv',
        sep="\t", usecols=['protein', 'scaffold', 'AMG KO', 'AMG KO name'])
    dramv_amgs = pd.read_table("results/DRAMv/distilled/amg_summary.tsv", 
        sep='\t', usecols=['gene', 'scaffold', 'gene_id','gene_description'])
...
def combine_sample_stats():
    ...
    for index, row in df1.iterrows():
        ...
        d = {
            'Sample': row['Sample'],
            'Contigs': (row['Contigs']),
            'Viral Contigs': (int(row['CheckV_Complete'] +  
    ...
    for index, row in df2.iterrows():
        ...
        my_df2.append(d)
    new_df1 = pd.DataFrame(my_df1, columns=['Sample', 'Contigs','Viral Contigs','QC Viral Contigs'])
    new_df2 = pd.DataFrame(my_df2, columns=['Sample', 'Contigs','Viral Contigs','QC Viral Contigs'])
    combined_df = new_df1.add(new_df2, fill_value=0)
    combined_df.set_index("Sample")
    combined_df.to_csv('./results/statistics/Combined_Sample_stats.tsv', sep="\t",  index=False)
...
def plot_sampleStats():
    ...
    df2 = pd.DataFrame(data, columns = ['Sample', 'Contigs','Viral Contigs','QC Viral Contigs']).set_index('Sample')
...
def plot_sampleStats_viral():
    ...
    """
    data = []
    df = pd .read_table('./results/statistics/Combined_Sample_stats.tsv', sep="\t", usecols=['Sample','Viral Contigs','QC Viral Contigs'])
    for index, row in df.iterrows():
        data.append(row.to_list())
    df2 = pd.DataFrame(data, columns = ['Sample','Viral Contigs','QC Viral Contigs']).set_index('Sample')
    # create figure and axis
    fig, ax = plt.subplots(figsize=(20, 20))
    # setting the axis' labels
    ax.set_ylabel('Number of Contigs',fontsize=32)
...
def relative_abundace_plot():
    ...
    fig.savefig('./results/statistics/plots/relative_abundance_plot.png', bbox_inches='tight')
    plt.clf()
...
def vOTU_families_plot():
    ...
    fig.savefig("./results/statistics/plots/Family_vOTU_BarChart.png", bbox_inches="tight")
    plt.clf()
...
def vOTU_Subfamily_plot():
    ...
    fig.savefig("./results/statistics/plots/Subfamily_vOTU_BarChart.png", bbox_inches="tight")
    plt.clf()
...
def vOTU_Genus_plot():
    ...
    fig.savefig("./results/statistics/plots/Genus_vOTU_BarChart.png", bbox_inches="tight")
    plt.clf()
...
def vOTU_CheckV_plot():
    ...
    fig.savefig("./results/statistics/plots/checkv_quality_vOTU_BarChart.png", bbox_inches="tight")
    plt.clf()
...
def vOTU_to_reads_mapping():
    ...
    for x in samples["sample"]:
        cV_df=pd.read_table("./results/checkv/vibrant/"+x+"/quality_summary.tsv", sep="\t")
        cV_combined_vibrant = pd.concat([cV_combined_vibrant, cV_df])

    cV_combined_virsorter2 = pd.DataFrame(columns=["contig_id", "provirus"])
    for x in samples["sample"]:
        cV_df=pd.read_table("./results/checkv/virsorter2/"+x+"/quality_summary.tsv", sep="\t")
        cV_combined_virsorter2 = pd.concat([cV_combined_virsorter2, cV_df])

    cV_provirus = pd.concat([cV_combined_vibrant.loc[cV_combined_vibrant["provirus"] == "Yes"], 
    ...
    cV_provirus =list(map(lambda s: re.sub(r'(\|\|.+|_fragment.+)', '', s), cV_provirus))
...
def gather_lytic():
    ...
    """
    provirus_df = pd.read_table("./results/statistics/vOTU_mapped_to_reads.tsv", sep="\t")
    combined_df = pd.read_table('./results/statistics/vOTU_stats_combined.tsv', sep="\t")
    lysogenic_df = pd.read_table('./results/vibrant/vOTU/VIBRANT_vOTU_derep95_combined/VIBRANT_results_vOTU_derep95_combined/VIBRANT_genome_quality_vOTU_derep95_combined.tsv', sep="\t")
...
def vOTU_provirus_plot():
    ...
    fig.savefig("./results/statistics/plots/provirus_vOTU_BarChart.png", bbox_inches="tight")
    plt.clf()
...
def vOTU_lysogenic_plot():
    ...
    fig.savefig("./results/statistics/plots/lysogenic_vOTU_BarChart.png", bbox_inches="tight")
    plt.clf()
